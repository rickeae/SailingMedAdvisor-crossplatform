...
function Install-ExecutableFromUrl {
    ...
    $tempDir = Join-Path $env:TEMP "sma_bootstrap"
...
try {
    ...
    Write-Section "SailingMedAdvisor Windows Bootstrap"
    ...
    if (-not (Test-Yes "Have you accepted the terms on both model pages?" $false)) {
        throw "Terms not accepted yet. Complete terms acceptance, then rerun bootstrap."
    }
    ...
    $lines = @(
        "# Auto-generated by scripts/bootstrap_windows.ps1",
        "FORCE_CUDA=0",
    ...
    Write-Host "Bootstrap stopped. Fix the issue above and rerun launch_windows_bootstrap.cmd." -ForegroundColor Red
    exit 1
}
